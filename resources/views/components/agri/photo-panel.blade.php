...
<div class="rounded-xl p-5 md:p-6 2xl:p-7" style="background:#f2f7ee;border:1px solid #dbe8d2;border-left:4px solid #1f6d3c">
  ...
  <div class="flex gap-3 mt-4">
    <button @click="runDiagnosis()" :disabled="diagnosing" class="px-5 py-2.5 2xl:px-6 2xl:py-3 rounded-lg text-[13.5px] 2xl:text-[14.5px] font-semibold text-white flex items-center gap-2 transition" :style="`background:#1f6d3c;box-shadow:0 2px 6px rgba(31,109,60,.3);opacity:${diagnosing ? .7 : 1}`">
    ...
    <button x-show="diagnosed" x-cloak @click="resetDiagnosis()" class="px-5 py-2.5 2xl:px-6 2xl:py-3 rounded-lg text-[13.5px] 2xl:text-[14.5px] font-semibold flex items-center gap-2 transition hover:opacity-85" style="border:1px solid #c1440e;color:#c1440e">
      ...
      <div class="flex gap-3 flex-1">
        <template x-for="(s, i) in symptomPages[symptomPage]" :key="i">
          <div class="flex-1 text-center">
            <div class="w-full h-28 md:h-32 2xl:h-36 rounded-lg flex items-center justify-center" style="background:linear-gradient(135deg,#e2efd9,#d6dccd)">
              <i data-lucide="leaf" class="w-8 h-8 md:w-9 md:h-9 2xl:w-10 2xl:h-10" style="color:#1f6d3c"></i>
            </div>
